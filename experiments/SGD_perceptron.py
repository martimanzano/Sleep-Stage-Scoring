...
### CREACION DE GRAFO DE COMPUTO
    ...
    with graph.as_default():
        ...
        n_hidden_1 = 200
        n_hidden_2 = 200
        n_input = config.sample_freq * config.epoch_duration_s
        ...
        def multilayer_perceptron(x_data, weights, biases):
            ...
            #layer_2 = tf.nn.dropout(layer_2, 0.5, seed=None)
    ...
    with tf.Session(graph=graph) as session:
        ...
        print("Stochastic Gradient Descent MultiLayer-Perceptron: Initializated")
        ...
        for training_x, training_y, validation_x, validation_y \
            ...
            for step in range(config.num_steps_GD):
                ...
                epochs_training_x = divide_in_epochs(training_x, labels_size_training_list,
                                                     config.sample_freq, config.epoch_duration_s)
                epochs_training_y = join_stages_hot_enc(training_y)

                offset = (step * config.SGD_batch_size) % (epochs_training_y.shape[0] - config.SGD_batch_size)
                batch_data = epochs_training_x[offset:(offset + config.SGD_batch_size), :]
                batch_labels = epochs_training_y[offset:(offset + config.SGD_batch_size), :]

                feed_dict_train = {tf_data_x: batch_data, tf_data_y: batch_labels}
                ...
                if step % 10 == 0:
                    ...
                    print("----------SGD STEP %i of %i FINISHED----------" % (step + 1, config.num_steps_GD))
